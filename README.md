...
- 转发的图片、视频、动画或文件会达到验证阈值；通过其他机器人发送的消息会增加风险分，消息内联按钮包含 URL、登录链接或 Web App 时会直接达到验证阈值。因此无需 OCR 也能识别常见的“转发广告图＋频道按钮”结构。
...
- 验证成功产生的临时放行状态有效 24 小时；管理员正常回复成功后，用户才会写入 `trusted-{UID}` 永久信任状态。旧版永久布尔值 `verified-{UID}` 不会被新版当作有效认证。
- 管理员对广告消息执行 `/block` 时，会同时撤销该 UID 的临时验证和永久信任，并保存规范化后的广告文本指纹 30 天；其他 UID 再次发送相同广告时会被静默屏蔽。对原消息执行 `/unblock` 会同时移除对应指纹，便于纠正误判。
...
| `verified-{UID}` | 新版 `{ version, verifiedAt, expiresAt }` 临时放行状态，有效期 24 小时；旧版永久布尔值不会被接受 |
| `captcha-{UID}` | 当前图标题的随机 ID、UID、轮次、目标名称、6 个图标选项、正确选项 ID、失败次数、聊天/消息信息和过期时间，10 分钟；必须更新时至少间隔 1100 毫秒 |
| `captcha-claim-{challengeId}` | Telegram 验证消息删除成功后的 UID 和 `claimedAt`；每个 challenge 只写一次，TTL 不超过当前验证会话 |
| `pending-message-{UID}` | 正常情况保留首次观察到的非 `/start` 消息 chat ID、message ID 和 `expiresAt`；创建时只写一次，之后自然过期 |
