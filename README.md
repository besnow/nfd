...
- 陌生用户首次发送 `/start`（或其他消息）时会收到双图标组合验证。机器人显示目标顺序，
  用户只需从随机排列的 6 个“双图标组合”按钮中点击一次；其中只有一个组合完全正确。
  验证题 2 分钟内有效，有效且已成功发送的题目存在时不会重复发送新题。
...
每分钟使用一个幂等的触发标记，因此并发读取延迟可能导致少计或延迟封禁，但不会把同一分钟的
并发超限重复累计成多个触发。它主要用于普通的机器人私聊防骚扰场景。
...
| `captcha-{UID}` | 当前题目的随机 ID、目标组合、6 个选项、正确选项 ID、连续失败次数、聊天/消息信息和过期时间，2 分钟 |
| `captcha-block-{UID}` | 连续验证失败后的静默限制，1 小时 |
| `rate-minute-{UID}-{窗口}` | 分钟窗口消息计数（键额外保留约 2 分钟） |
| `rate-hour-{UID}-{窗口}` | 小时窗口消息计数（键额外保留约 2 小时） |
| `rate-hit-{UID}-{分钟窗口}` | 某分钟窗口已经触发限速的幂等标记，1 小时；同一分钟最多形成一次 strike |
